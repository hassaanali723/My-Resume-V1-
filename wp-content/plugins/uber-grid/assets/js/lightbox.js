(function ($) {
    ...
    function Lightbox(element, options) {
        ...
        this.$item = '';
        this.$cont = '';
        if(jQuery('.pfhub_portfolio_portfolio_container').hasClass('view-grid') || jQuery('.pfhub_portfolio_portfolio_container').hasClass('view-masonry') || jQuery('.pfhub_portfolio_portfolio_container').hasClass('view-list') || jQuery('.portfolio-gallery-content').hasClass('view-content-slider')) {
    ...
    Lightbox.prototype.init = function () {
        ...
        if ($hash.indexOf('rlightbox') > 0) {
            ...
            $object.$body.addClass('pfhub-share');
            if (!$object.$body.hasClass('pfhub-on')) {
                setTimeout(function () {
                    $object.build($object.index);
                }, 900);
                $object.$body.addClass('pfhub-on');
            }
        }

        (($object.settings.preload > $object.$items.length) && ($object.settings.preload = $object.$items.length));

        $object.$items.on('click.pfhubcustom', function (event) {
    ...
    Lightbox.prototype.build = function (index) {
        ...
        $object.$cont.on('click.pfhub-container', function () {

            $object.$cont.removeClass($object.settings.classPrefix + 'hide-items');

        });

        $('.shareLook').on('click.pfhub-container', function(){
            $(this).css({'display' : 'none'});
            $('.pfhub-share-buttons').css({'display' : 'block'});
            /* setTimeout(function(){
             $('.shareLook').css({'display' : 'block'});
             $('.pfhub-share-buttons').css({'display' : 'none'});
             }, 9000);*/
        });
    ...
    Lightbox.prototype.structure = function () {
        ...
        $view = (this.settings.lightboxView === 'view6') ? 'pfhub-view6' : '';
        ...
        template = '<div class="' + this.settings.classPrefix + 'cont ">' +
            '<div class="pfhub-container pfhub-' + this.settings.lightboxView + ' pfhub-arrows_hover_effect-' + arrowHE + '">' +
        ...
        $(window).on('resize.pfhub-container', function () {
            setTimeout(function () {
                $object.calculateDimensions();
            }, 100);
        });
        ...
        if (this.settings.download) {
            ...
            switch (this.settings.lightboxView) {
                ...
                case 'view2':
                    ...
                    break;
                case 'view4':
                    $('<a id="' + $object.settings.classPrefix + 'download" target="_blank" download class="' + this.settings.classPrefix + 'download ' + $object.settings.classPrefix + 'icon">' + $download_bg + '</a>').insertBefore($('.pfhub-title'));;
                    break;
                case 'view5':
                case 'view6':
                    $('.pfhub-toolbar').append('<a id="' + $object.settings.classPrefix + 'download" target="_blank" download class="' + this.settings.classPrefix + 'download ' + $object.settings.classPrefix + 'icon">' + $download_bg_ + '</a>');
                    break;
            }
        }

        $arrows = $('.pfhub-arrows .pfhub-next, .pfhub-arrows .pfhub-prev');
        $next   = $('.pfhub-arrows .pfhub-next');
        $prev   = $('.pfhub-arrows .pfhub-prev');
        ...
        switch (this.settings.lightboxView) {
            ...
            case 'view3':
                ...
                $next.css({'right' : '20px'});
                $prev.css({'right' : '55px'});
                $('.pfhub-title').css({'text-align' : 'left','border-top' : '1px solid #999'});
                $('.pfhub-close').css({'margin-right' : '45%'});
                $('.pfhub-overlay, .pfhub-toolbar, .pfhub-title, .pfhub-next, .pfhub-prev').css({'background' : 'rgba(255, 255, 255, 1)'});
                $('.pfhub-title, .shareLook').css({'color' : '#999'});
                $('.pfhub-toolbar').css({'border-bottom' : '1px solid #999'});
                $('.pfhub-toolbar .pfhub-icon, .pfhub-arrows .pfhub-icon').addClass('pfhub-icon0');
                break;
        }

        this.prevScrollTop = $(window).scrollTop();

        $object.objects.content = $('.pfhub-container');
        ...
        if(portfolioGalleryDisableRightClickLightbox == 'on') {
            setTimeout(function () {
                $('.pfhub-container').bind('contextmenu', function () {
                    return false;
                });
            }, 50);
        }
        ...
        if (this.settings.showCounter) {
            switch (this.settings.lightboxView) {
                ...
                default:
                    ...
                    $('#pfhub-counter').css({'padding-left' : '23px'});
                    break;
                case 'view2':
                case 'view4':
    ...
    Lightbox.prototype.setTitle = function (index) {
        ...
        this.$cont.find('.' + this.settings.classPrefix + 'title').html('<div class="pfhub-title-text">'+$title+'</div>');

        (($object.settings.lightboxView === 'view2') && $('.pfhub-title-text').css({'width' : '100%'}));
    ...
    Lightbox.prototype.setDescription = function (index) {
        ...
        this.$cont.find('.' + this.settings.classPrefix + 'description').html('<div class="pfhub-description-text" title="' + $description + '">' + $description + '</div>');
    };
    ...
    Lightbox.prototype.socialShare = function () {
        ...
        var shareButtons = '<ul class="pfhub-share-buttons">';
        shareButtons += $object.settings.share.facebookButton ? '<li><a title="Facebook" id="pfhub-share-facebook" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.twitterButton ? '<li><a title="Twitter" id="pfhub-share-twitter" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.googleplusButton ? '<li><a title="Google Plus" id="pfhub-share-googleplus" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.pinterestButton ? '<li><a title="Pinterest" id="pfhub-share-pinterest" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.linkedinButton ? '<li><a title="Linkedin" id="pfhub-share-linkedin" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.tumblrButton ? '<li><a title="Tumblr" id="pfhub-share-tumblr" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.redditButton ? '<li><a title="Reddit" id="pfhub-share-reddit" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.bufferButton ? '<li><a title="Buffer" id="pfhub-share-buffer" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.diggButton ? '<li><a title="Digg" id="pfhub-share-digg" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.vkButton ? '<li><a title="VK" id="pfhub-share-vk" target="_blank"></a></li>' : '';
        shareButtons += $object.settings.share.yummlyButton ? '<li><a title="Yummly" id="pfhub-share-yummly" target="_blank"></a></li>' : '';
        shareButtons += '</ul>';
        ...
        setTimeout(function () {
            $('#pfhub-share-facebook').attr('href', 'https://www.facebook.com/sharer/sharer.php?u=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-twitter').attr('href', 'https://twitter.com/intent/tweet?text=&url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-googleplus').attr('href', 'https://plus.google.com/share?url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-pinterest').attr('href', 'http://www.pinterest.com/pin/create/button/?url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-linkedin').attr('href', 'http://www.linkedin.com/shareArticle?mini=true&amp;url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-tumblr').attr('href', 'http://www.tumblr.com/share/link?url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-reddit').attr('href', 'http://reddit.com/submit?url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-buffer').attr('href', 'https://bufferapp.com/add?url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-digg').attr('href', 'http://www.digg.com/submit?url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-vk').attr('href', 'http://vkontakte.ru/share.php?url=' + (encodeURIComponent(window.location.href)));
            $('#pfhub-share-yummly').attr('href', 'http://www.yummly.com/urb/verify?url=' + (encodeURIComponent(window.location.href)));
        }, 200);
    };
    ...
    Lightbox.prototype.loadContent = function (index, rec, delay) {
        ...
        if ($object.settings.watermark) {
            ...
            if (isImg()) {
                if(performance.navigation.type === 1 && !jQuery('.pfhub_portfolio_portfolio_container').hasClass('view-image-grid') && this.settings.socialSharing){
                    src = $object.$items.eq(index).attr('href');
                } else {
        ...
        if (!$object.$item.eq(index).hasClass($object.settings.classPrefix + 'loaded')) {
            if (isVideo) {
                ...
                $object.$element.trigger('hasVideo.pfhub-container', [index, src]);
            } else {
                $object.$item.eq(index).prepend('<div class="pfhub-img-wrap"><img class="pfhub-object pfhub-image watermark" src="' + src + '" /></div>');
            }

            $object.$element.trigger('onAferAppendSlide.pfhub-container', [index]);

            $object.$item.eq(index).addClass($object.settings.classPrefix + 'loaded');
        }

        $object.$item.eq(index).find('.' + $object.settings.classPrefix + 'object').on('load.pfhub-container error.pfhub-container', function () {
    ...
    Lightbox.prototype.slide = function (index, fromSlide, fromThumb) {
        ...
        this.$element.trigger('onBeforeSlide.pfhub-container', [prevIndex, index, fromSlide, fromThumb]);
        ...
        $object.$element.trigger('onAfterSlide.pfhub-container', [prevIndex, index, fromSlide, fromThumb]);
    ...
    Lightbox.prototype.slideShow = function () {
        ...
        switch (this.settings.lightboxView) {
            ...
            case 'view3':
                $toolbar.append('<span class="' + $object.settings.classPrefix + 'autoplay-button ' + $object.settings.classPrefix + 'icon">' + $play_bg + $pause_bg + '</span>');
                $('.pfhub-toolbar .pfhub-icon').addClass('pfhub-icon0');
                break;
            case 'view4':
                $('<span class="' + $object.settings.classPrefix + 'autoplay-button ' + $object.settings.classPrefix + 'icon">' + $play_bg + $pause_bg + '</span>').insertBefore($('.pfhub-title'));
                $('.pfhub-toolbar .pfhub-icon').addClass('pfhub-icon0');
                break;
        }
        ...
        $object.$cont.find('.' + $object.settings.classPrefix + 'autoplay-button').on('click.pfhub-container', function () {
    ...
    Lightbox.prototype.startSlide = function () {
        var $object = this;
        $object.$cont.addClass('' + $object.settings.classPrefix + 'show-autoplay');
        $('.pfhub-autoplay-button > .pause_bg').css({'display' : 'inline-block'});
        $('.pfhub-autoplay-button > .play_bg').css({'display' : 'none'});
    ...
    Lightbox.prototype.stopSlide = function () {
        ...
        $('.pfhub-autoplay-button > .pause_bg').css({'display' : 'none'});
        $('.pfhub-autoplay-button > .play_bg').css({'display' : 'inline-block'});
    };

    Lightbox.prototype.addKeyEvents = function () {
        var $object = this;
        if (this.$items.length > 1) {
            $(window).on('keyup.pfhub-container', function (e) {
                if ($object.$items.length > 1) {
                    if (e.keyCode === 37) {
                        e.preventDefault();
        ...
        $(window).on('keydown.pfhub-container', function (e) {
    ...
    Lightbox.prototype.arrow = function () {
        var $object = this;
        this.$cont.find('.' + $object.settings.classPrefix + 'prev').on('click.pfhub-container', function () {
            $object.goToPrevSlide();
        });

        this.$cont.find('.' + $object.settings.classPrefix + 'next').on('click.pfhub-container', function () {
            $object.goToNextSlide();
        });
    };
    ...
    Lightbox.prototype.closeGallery = function () {
        ...
        this.$cont.find('.' + $object.settings.classPrefix + 'close').on('click.pfhub-container', function () {
            $object.destroy();
        });

        if ($object.settings.overlayClose) {

            $object.$cont.on('mousedown.pfhub-container', function (e) {

                mousedown = ($(e.target).is('.' + $object.settings.classPrefix + 'cont') || $(e.target).is('.' + $object.settings.classPrefix + 'item ') || $(e.target).is('.' + $object.settings.classPrefix + 'img-wrap'));

            });

            $object.$cont.on('mouseup.pfhub-container', function (e) {
    ...
    Lightbox.prototype.destroy = function (d) {
        ...
        this.$element.off('.pfhub-container.tm');

        $(window).off('.pfhub-container');
    ...
    Video.prototype.init = function () {
        ...
        $object.core.$element.on('hasVideo.pfhub-container.tm', function (event, index, src) {
        ...
        $object.core.$element.on('onAferAppendSlide.pfhub-container.tm', function (event, index) {
        ...
        $object.core.$element.on('onBeforeSlide.pfhub-container.tm', function (event, prevIndex, index) {

            var $videoSlide = $object.core.$item.eq(prevIndex),
                youtubePlayer = $videoSlide.find('.pfhub-youtube').get(0),
                vimeoPlayer = $videoSlide.find('.pfhub-vimeo').get(0);
        ...
        $object.core.$element.on('onAfterSlide.pfhub-container.tm', function (event, prevIndex) {
