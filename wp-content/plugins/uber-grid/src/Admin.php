...
class Admin
{
    ...
    {
        $this->pages[] = add_menu_page(__('PortfolioHub Portfolio Gallery', 'pfhub_portfolio'), __('Portfolio Gallery', 'pfhub_portfolio'), 'delete_pages', 'portfolios_pfhub_portfolio_portfolio', array(
            AdminGrids::class,
            'load_portfolio_page'
        ), 'dashicons-layout');
        $this->pages[] = add_submenu_page('portfolios_pfhub_portfolio_portfolio', __('Portfolios', 'pfhub_portfolio'), __('Portfolios', 'pfhub_portfolio'), 'delete_pages', 'portfolios_pfhub_portfolio_portfolio', array(
            AdminGrids::class,
            'load_portfolio_page'
        ));

        $this->pages[] = add_submenu_page('portfolios_pfhub_portfolio_portfolio', __('Settings', 'pfhub_portfolio'), __('Settings', 'pfhub_portfolio'), 'delete_pages', 'Options_portfolio_styles', array(
            $this,
            'settings'
        ));
        $this->pages[] = add_submenu_page('portfolios_pfhub_portfolio_portfolio', __('Lightbox Settings', 'pfhub_portfolio'), __('Lightbox Settings', 'pfhub_portfolio'), 'delete_pages', 'Options_portfolio_lightbox_styles', array(
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] == 'portfolios_pfhub_portfolio_portfolio') {
            if (isset($_GET['task'])) {
    ...
    public static function add_portfolio()
    {
        ...
        $apply_portfolio_safe_link = wp_nonce_url(
            'admin.php?page=portfolios_pfhub_portfolio_portfolio&id=' . $wpdb->insert_id . '&task=apply',
    ...
    public static function add_video()
    {
        ...
        $apply_portfolio_safe_link = wp_nonce_url(
            'admin.php?page=portfolios_pfhub_portfolio_portfolio&id=' . $id . '&task=apply',
            'apply_portfolio_' . $id,
            'pfhub_portfolio_apply_portfolio_nonce'
        );
        $apply_portfolio_safe_link = htmlspecialchars_decode($apply_portfolio_safe_link);
    ...
    public static function edit_video()
    {
        ...
        $apply_portfolio_safe_link = wp_nonce_url(
            'admin.php?page=portfolios_pfhub_portfolio_portfolio&id=' . $grid_id . '&task=apply',
    ...
    public static function remove_portfolio()
    {
        ...
        header('Location: admin.php?page=portfolios_pfhub_portfolio_portfolio');
    }
    ...
    public static function duplicate_portfolio()
    {
        if (isset($_GET['page']) && $_GET['page'] == 'portfolios_pfhub_portfolio_portfolio') {
            ...
            if ($task && $task == 'duplicate_portfolio_gallery') {
                ...
                $id = absint($_GET["id"]);
                if (!isset($_REQUEST['pfhub_portfolio_duplicate_nonce']) || !wp_verify_nonce($_REQUEST['pfhub_portfolio_duplicate_nonce'], 'pfhub_portfolio_portfolio_duplicate_nonce' . $id)) {
                ...
                $wpdb->query($query);
                wp_redirect('admin.php?page=portfolios_pfhub_portfolio_portfolio');
            }
        }
    }
