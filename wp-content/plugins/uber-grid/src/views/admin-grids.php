...
<div class="wrap">
    ...
    <div id="poststuff">
        <div id="portfolios-list-page">
            <form method="post" onkeypress="doNothing()" action="admin.php?page=portfolios_pfhub_portfolio_portfolio"
			...
			<div class="alignleft actions">
				<input type="button" value="Search" onclick="document.getElementById(\'page_number\').value=\'1\'; document.getElementById(\'serch_or_not\').value=\'search\';
				 ...
				 <input type="button" value="Reset" onclick="window.location.href=\'admin.php?page=portfolios_pfhub_portfolio_portfolio\'" class="button-secondary action">
			</div>';
                ...
                <table class="wp-list-table widefat fixed pages">
                    ...
                    for ( $i = 0; $i < count( $rows ); $i ++ ) {
                        ...
                        $delete_portfolio_safe_link = wp_nonce_url(
                            'admin.php?page=portfolios_pfhub_portfolio_portfolio&task=remove_portfolio&id=' . $rows[ $i ]->id,
                            'remove_portfolio_' . $rows[ $i ]->id,
                            'pfhub_portfolio_remove_portfolio_nonce'
                        );

                        $pfhub_portfolio_pfhub_portfolio_nonce_duplicate_portfolio = wp_create_nonce( 'pfhub_portfolio_portfolio_duplicate_nonce' . $rows[ $i ]->id );
                        ?>
                        <tr>
                            <td><?php echo $rows[ $i ]->id; ?></td>
                            <td>
                                <a href="<?php echo esc_url( 'admin.php?page=portfolios_pfhub_portfolio_portfolio&task=edit_cat&id=' . $rows[ $i ]->id ); ?>"><?php echo esc_html( stripslashes( $rows[ $i ]->name ) ); ?></a>
                            </td>
                            <td>(<?php if ( ! ( $pr_count ) ) {
                                    echo '0';
                                } else {
                            ...
                            <td>
                                [pfhub_portfolio_portfolio id="<?php echo $rows[ $i ]->id; ?>"]
                            </td>
                            <td>
                                <a href="admin.php?page=portfolios_pfhub_portfolio_portfolio&task=duplicate_portfolio_gallery&id=<?php echo $rows[ $i ]->id; ?>&pfhub_portfolio_duplicate_nonce=<?php echo $pfhub_portfolio_pfhub_portfolio_nonce_duplicate_portfolio; ?>"
