...
<div id="pfhub_portfolio_portfolio" style="display:none;" class="post-content">
    <div class="portfolio_insert_shortcode_popup">
        <h3>Select PortfolioHub Portfolio Gallery to insert into post</h3>
        <?php
        global $wpdb;
        $query        = "SELECT * FROM " . $wpdb->prefix . "pfhub_portfolio_grids";
        $firstrow     = $wpdb->get_row( $query );
        $container_id = 'pfhub_portfolio_portfolio';
        ...
        if ( count( $shortcodeportfolios ) ) {
            echo "<select id='pfhub_portfolio_portfolio-select' class='select_portfolio_list'   name='pfhub_portfolio_grid_id' change-view-nonce='" . $shortecode_change_view_nonce . "'>";
